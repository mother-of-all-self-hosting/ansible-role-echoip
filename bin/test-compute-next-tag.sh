...
# the script does. Two decoys sit above `echoip_version`, both of which a
...
# upstream version to speak of, and a `v0.0.0-N` release history that has
...
# `v0.0.0-N` series past the two-digit boundary it is already at.
scenario 'Role changes with no upstream version to speak of'
expect 'task edit' v0.0.0-15 "$(merge "$edit_task")"
expect 'template'  v0.0.0-16 "$(merge "$edit_template")"
...
expect 'a task'   v0.0.0-15  "$(merge "$edit_task")"

# Should echoip ever start publishing versions and this role pin one, the tag
...
merge "$pin_version" > /dev/null
expect 'unpin' v0.0.0-15 "$(merge "$unpin_version && $edit_task")"

# -9 must not be read as the newest release when -14 exists. This repository
# is already past that boundary, so a lexicographic sort would tag on top of
# an existing release here rather than after it.
